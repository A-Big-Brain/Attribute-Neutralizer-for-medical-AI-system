...
def run_model(da_tr, folder, model_num, bat_num, ep, tr_it, te_it, pr_it, le_rate=0.0005):
    # create folder
    sa_str = spb.com_mul_str(folder.split('_')[:-6] + [model_num, bat_num, ep, tr_it, te_it])
    sa_fo = spb.mo_pa + 'save_results/' + sa_str + '/'
    os.mkdir(sa_fo)

    # create datase
    da = spd.dataset(da_tr, folder, bat_num, -1)
    ...
    model = spn.get_model(da.lab_na.shape[0], model_num)
    model.to(device)
    transforms = spn.get_transforms()
    transforms.to(device)
    loss_fn = nn.BCEWithLogitsLoss(pos_weight = da.cal_w)
    loss_fn.to(device)
