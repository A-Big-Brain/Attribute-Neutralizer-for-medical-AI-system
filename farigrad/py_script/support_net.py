...
def train(da, model, transforms, loss_fn, optimizer, device, it_num, pr_it):
    ...
    for i in range(it_num):
        X, y1, z, _, _ = da.get_bat_data('tr')
        X, y1, z, = X.to(device), y1.type(torch.LongTensor).to(device), z.type(torch.LongTensor).to(device)
        ...
        # loss
        loss = loss_fn(log, y1, z, mode="train")
...
def test(da, model, transforms, loss_fn, ty_str, device, it_num, pr_it):
    ...
    with torch.no_grad():
        for i in range(it_num):
            X, y1, z, _, idx = da.get_bat_data(ty_str)
            X, y1, z, = X.to(device), y1.type(torch.LongTensor).to(device), z.type(torch.LongTensor).to(device)
            ...
            if i % pr_it == 0:
                print('validation', i, datetime.datetime.now(), loss.item())
