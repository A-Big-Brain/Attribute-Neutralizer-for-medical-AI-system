...
def run_model(da_tr, att_str, model_num, bat_num, ep, tr_it, te_it, pr_it, le_rate=0.0005):
    # create folder
    sa_str = spb.com_mul_str([da_tr, att_str, model_num, bat_num, ep, tr_it, te_it])
    sa_fo = spb.mo_pa + 'save_results/' + sa_str + '/'
    os.mkdir(sa_fo)

    # create datase
    da = spd.dataset(da_tr, att_str, bat_num, -1)
