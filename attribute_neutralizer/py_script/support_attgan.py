...
from support_net import LinearBlock, Conv2dBlock, ConvTranspose2dBlock
...
class Generator(nn.Module):
    def __init__(self, enc_dim=64, enc_layers=5, enc_norm_fn='batchnorm', enc_acti_fn='lrelu',
                 ...
                 n_attrs=2, shortcut_layers=1, inject_layers=0, img_size=256):
        ...
        for i in range(enc_layers):
            ...
            layers += [Conv2dBlock(n_in, n_out, (6, 6), stride=2, padding=2, norm_fn=enc_norm_fn, acti_fn=enc_acti_fn)]
            n_in = n_out
        ...
        for i in range(dec_layers):
            if i < dec_layers - 1:
                ...
                layers += [ConvTranspose2dBlock(n_in, n_out, (6, 6), stride=2, padding=2, norm_fn=dec_norm_fn, acti_fn=dec_acti_fn)]
                n_in = n_out
                n_in = n_in + n_in // 2 if self.shortcut_layers > i else n_in
                n_in = n_in + n_attrs if self.inject_layers > i else n_in
            else:
                layers += [ConvTranspose2dBlock(n_in, 1, (6, 6), stride=2, padding=2, norm_fn='none', acti_fn='tanh')]
...
class Discriminators(nn.Module):
    ...
    def __init__(self, dim=64, norm_fn='instancenorm', acti_fn='lrelu', n_attrs=2,
                 fc_dim=1024, fc_norm_fn='none', fc_acti_fn='lrelu', n_layers=5, img_size=256):
        ...
        for i in range(n_layers):
            ...
            layers += [Conv2dBlock(
                n_in, n_out, (6, 6), stride=2, padding=2, norm_fn=norm_fn, acti_fn=acti_fn
            )]
            n_in = n_out
        self.conv = nn.Sequential(*layers)
        self.fc_adv = nn.Sequential(
            LinearBlock(dim * 2**(n_layers-1) * self.f_size * self.f_size, fc_dim, fc_norm_fn, fc_acti_fn),
...
class AttGAN():
    def __init__(self, args):
        ...
        self.update_lambda_rate = args.update_lambda_rate
        self.add_noise_to_dfloss = args.add_noise_to_dfloss
        self.add_noise_to_dcloss = args.add_noise_to_dcloss
        ...
        self.D = Discriminators(
            args.dis_dim, args.dis_norm, args.dis_acti, args.n_attrs,
    ...
    def trainG(self, img_a, att_a, att_a_, att_b, att_b_):
        ...
        d_fake, dc_fake = self.D(img_fake + torch.randn_like(img_fake)*0.1)
    ...
    def trainD(self, img_a, att_a, att_a_, att_b, att_b_):
        ...
        d_real, dc_real = self.D(img_a + torch.randn_like(img_a)*0.1)
        d_fake, dc_fake = self.D(img_fake + torch.randn_like(img_fake)*0.1)
        ...
        if self.mode == 'dcgan':  # sigmoid_cross_entropy
            real_gd = torch.ones_like(d_real)
            fake_gd = torch.zeros_like(d_fake)
            if self.add_noise_to_dfloss == 'yes':
                real_gd[:int(real_gd.shape[0] * 0.05)] = 0
                fake_gd[:int(fake_gd.shape[0] * 0.05)] = 1
            df_loss = F.binary_cross_entropy_with_logits(d_real, real_gd) + F.binary_cross_entropy_with_logits(d_fake, fake_gd)
            df_gp = gradient_penalty(self.D, img_a)

        if self.add_noise_to_dcloss == 'yes':
            att_a_[:int(att_a_.shape[0] * 0.05), :] = -att_a_[:int(att_a_.shape[0] * 0.05), :]
    ...
    def update_lambda(self):
        # self.lambda_2 = self.lambda_2 * self.update_lambda_rate
        self.lambda_2 = self.lambda_2 + self.update_lambda_rate